...
impl Node {
    ...
    pub fn find_node_by_id_and_update(&mut self, target_id: u128, text: &str, level: f32, mut min_date: u32, mut max_date: u32, mut has_dates: bool) 
    -> (bool, u32, u32, bool) {
        if self._id == target_id {
            self._text = text.to_string();
            self._level = level; 
            (self._md_text, self._skimmed_text, self._date1, self._date2, self._pad, min_date, max_date, has_dates) = parse_content(&self._text, min_date, max_date);
            return (true, min_date, max_date, has_dates); 
        }

        if self._first_child.is_some() {
            let res = self._first_child.as_mut().unwrap().find_node_by_id_and_update(target_id, &text, level, min_date, max_date, has_dates);
            if res.0 {
                return res;
            }
        }
        ...
        if self._next_sibling.is_some() {
            ...
            loop {
                let res = current_sibling.find_node_by_id_and_update(target_id, &text, level, min_date, max_date, has_dates);
                if res.0 {
                    return res;
                } else {
                    if current_sibling._next_sibling.is_some() {
        ...
        return (false, min_date, max_date, has_dates);
    }

    pub fn find_node_by_id_and_create(&mut self, key_id: u128, target_id: u128, text: &str, level: f32, mut min_date: u32, mut max_date: u32, mut has_dates: bool) 
    -> (bool, u32, u32, bool) {
        if self._id == target_id {
            ...
            (current._md_text, current._skimmed_text, current._date1, current._date2, current._pad, min_date, max_date, has_dates) = parse_content(text, min_date, max_date);
        ...
        if self._first_child.is_some() {
            let res = self._first_child.as_mut().unwrap().find_node_by_id_and_create(key_id, target_id, &text, level, min_date, max_date, has_dates);
            if res.0 {
                return res;
            }
        }
        ...
        if self._next_sibling.is_some() {
            let mut current_sibling = self._next_sibling.as_mut().unwrap();
            loop {
                let res = current_sibling.find_node_by_id_and_create(key_id, target_id, &text, level, min_date, max_date, has_dates);
                if res.0{
                    return res;
                } else {
                    if current_sibling._next_sibling.is_some() {
